...
def test_markdown_folder_node_reload_basic(fs):
...
def test_markdown_folder_node_reload_with_preamble(fs):
...
def test_markdown_folder_node_reload_invalid_filename_raises(fs):
    ...
    with pytest.raises(Exception):
        MarkdownFolderNode(file_path=Path("/tmp/test.mdf"))


def test_markdown_folder_node_reload_ignores_non_mdp(fs):
...
def test_markdown_folder_node_get_text(fs):
...
def test_markdown_folder_node_set_text(fs):
    """测试 set_text 修改内部树结构"""
    fs.create_dir("/tmp/test.mdf")
    fs.create_file("/tmp/test.mdf/1_Intro.mdp", contents="## 1.1 Opening\nContent")
    node = MarkdownFolderNode(file_path=Path("/tmp/test.mdf"))
...
def test_markdown_folder_node_save_creates_file(fs):
...
def test_markdown_folder_node_save_renames_file(fs):
...
def test_markdown_folder_node_save_deletes_file(fs):
...
def test_markdown_folder_node_save_preamble(fs):
...
def test_markdown_folder_node_save_no_preamble_deletes_0(fs):
...
def test_markdown_folder_node_save_rewrites_existing(fs):
    """测试 save：重写既有文件的内容"""
    fs.create_dir("/tmp/test.mdf")
    fs.create_file(
        "/tmp/test.mdf/1_Intro.mdp",
...
def test_markdown_folder_node_reload_after_modification(fs):
...
def test_markdown_folder_node_round_trip(fs):
